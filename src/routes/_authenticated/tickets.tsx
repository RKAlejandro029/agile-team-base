  ...
  const create = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("tickets").insert({
        title, description, priority, created_by: user!.id, assigned_to: user!.id,
      });
  ...
  const addComment = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("ticket_updates").insert({
        ticket_id: detailId, user_id: user!.id, content: comment.trim(),
      });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {role === "admin" ? "All tickets across the team." : "Track your ongoing tasks."}
          </p>
        </div>
      ...
      <div className="grid gap-3">
        {filtered.map((t) => {
          const creator = (t as unknown as { creator: { full_name: string; email: string } | null }).creator;
          return (
            <Card key={t.id} className="cursor-pointer hover:border-primary/50 transition" onClick={() => setDetailId(t.id)}>
              <CardContent className="p-4 flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="font-medium truncate">{t.title}</p>
                    <Badge className={cn("capitalize", priorityColor[t.priority as Priority])} variant="secondary">{t.priority}</Badge>
                    <Badge className={cn("capitalize", statusColor[t.status as Status])} variant="secondary">{t.status.replace("_", " ")}</Badge>
                  </div>
                  {t.description && <p className="text-sm text-muted-foreground line-clamp-2">{t.description}</p>}
                  <p className="text-xs text-muted-foreground mt-1">
                    By {creator?.full_name || creator?.email} · Updated {format(new Date(t.updated_at), "MMM d, h:mm a")}
                  </p>
                </div>
        ...
        {filtered.length === 0 && <p className="text-center text-muted-foreground py-8">No tickets.</p>}
      </div>
        ...
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          ...
          {detail && (
            <div className="space-y-4 mt-4">
              <div className="flex gap-2">
                <Badge className={cn("capitalize", priorityColor[detail.priority as Priority])} variant="secondary">{detail.priority}</Badge>
                <Badge className={cn("capitalize", statusColor[detail.status as Status])} variant="secondary">{detail.status.replace("_", " ")}</Badge>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {detailQ.data?.map((u) => {
                    const p = (u as unknown as { profiles: { full_name: string; email: string } | null }).profiles;
                    return (
                      <div key={u.id} className="text-sm border-l-2 border-primary/40 pl-3 py-1">
                        <p className="text-xs text-muted-foreground">{p?.full_name || p?.email} · {format(new Date(u.created_at), "MMM d, h:mm a")}</p>
                        <p>{u.content}</p>
                      </div>
                    );
                  })}
                  {detailQ.data?.length === 0 && <p className="text-xs text-muted-foreground">No updates yet.</p>}
                </div>
              </div>
              <form className="space-y-2" onSubmit={(e) => { e.preventDefault(); addComment.mutate(); }}>
                <Textarea placeholder="Add an update..." value={comment} onChange={(e) => setComment(e.target.value)} />
                <Button type="submit" size="sm" disabled={!comment.trim()}>Post update</Button>
              </form>
            </div>
          )}
        </SheetContent>
